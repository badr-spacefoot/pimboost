...
export function suggestMapping(
  ...
  const sourceExact = relevantMemory.find(
    (entry) => entry.sourceName === sourceName && entry.sourceValueNormalized === normalized,
  );
  if (sourceExact) return { targetValue: sourceExact.targetValue, confidenceScore: 1, reason: 'source-history' };

  const exact = relevantMemory.find((entry) => !entry.sourceName && entry.sourceValueNormalized === normalized);
  ...
  if (bestSimilarity && bestSimilarity.score >= 0.78) {
    return {
      targetValue: bestSimilarity.entry.targetValue,
      confidenceScore: bestSimilarity.entry.sourceName === sourceName ? 0.9 : 0.8,
      reason: bestSimilarity.entry.sourceName === sourceName ? 'source-history' : 'similarity',
    };
  }
